// SENTCOR v4 — Chat (Realtime + compact support)
(function(){
  ...
  async function loadMessages(chId){
    ...
    else msgs.forEach(m=>S.ui.appendMessage(m,pCache));
    ...
    sub=sb.channel("msgs-"+chId).on("postgres_changes",{event:"INSERT",schema:"public",table:"messages",filter:"channel_id=eq."+chId},async p=>{
      const m=p.new;if(!pCache[m.sender_id])await fetchProfiles([m.sender_id]);if(curCh===chId)S.ui.appendMessage(m,pCache)
    }).subscribe()
  }
  ...
  async function fetchProfiles(ids){
    const miss=ids.filter(id=>!pCache[id]);if(!miss.length)return;
    const{data}=await sb.from("profiles").select("id,username,display_name,avatar_url,status,game_status,custom_status").in("id",miss);
  ...
  async function loadDMs(friendId){
    curCh=null;unsub();S.ui.resetCompact();
    ...
    const ids=[...new Set((msgs||[]).map(m=>m.sender_id))];await fetchProfiles(ids);
    const ct=document.getElementById("chat-messages");if(!ct)return;
    ct.innerHTML="";if(!msgs||!msgs.length)ct.innerHTML='<div class="empty-state"><i class="fa-solid fa-comments" style="font-size:40px;opacity:0.3;"></i><h3>Начните общение!</h3><p>Здесь будут личные сообщения.</p></div>';
    else msgs.forEach(m=>S.ui.appendMessage(m,pCache));
    ct.scrollTop=ct.scrollHeight;
    sub=sb.channel("dms-"+friendId).on("postgres_changes",{event:"INSERT",schema:"public",table:"direct_messages",filter:"sender_id=eq."+friendId},async p=>{
      const m=p.new;if(!pCache[m.sender_id])await fetchProfiles([m.sender_id]);S.ui.appendMessage(m,pCache)
    }).subscribe()
  }
