...
(function(){
  ...
  async function sendMessage(chId,content){
    ...
    if(!error&&data&&data[0]){
      const m=data[0];if(!pCache[m.sender_id])await fetchProfiles([m.sender_id]);S.ui.appendMessage(m,pCache)
    }
    return{data,error}
  }
