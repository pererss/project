...
// SENTCOR v9.3 — Voice (Refactored for Stability & Readability)
(function() {
    "use strict";

    const S = window.SENTCOR;
    if (!S || !S.sb || !window.LivekitClient) {
        console.error("SENTCOR base, Supabase client, or LiveKit client is not loaded!");
        return;
    }
    const sb = S.sb;
    ...
    S.voice = {
        ...
        leave,
        listenForCalls: listenForIncomingCalls,
        get isConnected() { return room && room.state === "connected"; }
    };
