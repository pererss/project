...
(function(){
  ...
  async function sendRequest(uid){
    ...
    var r=await c.from('friend_requests').insert(window.S.utils.cleanPayload({from_user:u.id,to_user:uid,status:'pending'}));if(r.error)throw r.error;return{success:true};}catch(e){console.error('[SentCor] sendRequest:',e);return{error:e.message};}
  }
  ...
  async function respondRequest(id,accept){
    ...
    return{success:true};}catch(e){console.error('[SentCor] respondRequest:',e);return{error:e.message};}
  }
  ...
  async function removeFriend(fid){
    ...
    if(r.error)throw r.error;return{success:true};}catch(e){console.error('[SentCor] removeFriend:',e);return{error:e.message};}
  }

  function sub(){if(_sub)return;try{var c=gS();if(!c)return;c.channel('public:friend_requests').on('postgres_changes',{event:'*',schema:'public',table:'friend_requests'},function(){if(window.S.friends.onUpdate)window.S.friends.onUpdate();}).subscribe();_sub=true;}catch(e){console.error('[SentCor] friends subscribe:',e);}}
