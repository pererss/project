...
(function(){
  ...
  async function showPage(){
    ...
    S.ui.setMainContent(`<div class="main-content" style="display:flex;align-items:center;justify-content:center;"><div style="text-align:center;color:var(--text-muted);"><i class="fa-solid fa-user-group" style="font-size:48px;opacity:0.3;display:block;margin-bottom:16px;"></i><h3>Выберите друга</h3><p style="font-size:13px;">Нажмите на друга в списке слева, чтобы увидеть его профиль</p></div></div>`);
    S.ui.clearMembers()
  }
  ...
  function renderSubPanel(){
    ...
    const afb=document.getElementById("add-friend-btn");if(afb)afb.addEventListener("click",showAddModal);
  ...
  function openDetail(fid){
    ...
    S.ui.renderFriendInfo(f);
    // Events
    const menuBtn=document.getElementById("fd-menu-btn");if(menuBtn)menuBtn.addEventListener("click",e=>{
      e.stopPropagation();
    ...
    const closeBtn=document.getElementById("fd-close");if(closeBtn)closeBtn.addEventListener("click",()=>{showPage()});
  ...
  function openDMView(friend){
    ...
    S.ui.renderFriendInfo(friend);
    S.chat.loadDMs(friend.id)
  }

  async function toggleMute(fid){const f=S.friendsList.find(x=>x.id===fid);if(!f)return;const nm=!f.muted;await sb.from("friends").update({muted:nm}).eq("user_id",S.user.id).eq("friend_id",fid);f.muted=nm;S.ui.toast(nm?"🔕 Без звука":"🔔 Звук вкл.","info");if(document.getElementById("fd-menu-btn"))openDetail(fid)}
  ...
  async function unblock(uid){await sb.from("blocked_users").delete().eq("blocker_id",S.user.id).eq("blocked_id",uid);S.ui.toast("Разблокирован","success");await loadAll();if(document.getElementById("fd-menu-btn"))openDetail(uid);else renderSubPanel()}
  ...
  function openDM(fid){const f=S.friendsList.find(x=>x.id===fid);if(!f)return;openDetail(fid);setTimeout(()=>{const btn=document.getElementById("fd-dm-btn");if(btn)btn.click()},150)}
