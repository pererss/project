// SENTCOR v5 — Friends (fixed acceptReq, ⋮ on members)
(function(){
  ...
  async function showPage(){
    ...
    S.ui.setSubPanelHeader("Друзья");await loadAll();renderSubPanel();
    S.ui.setMainContent(`<div class="main-content" style="display:flex;align-items:center;justify-content:center;"><div style="text-align:center;color:var(--text-muted);"><i class="fa-solid fa-user-group" style="font-size:48px;opacity:0.3;display:block;margin-bottom:16px;"></i><h3>Выберите друга</h3><p style="font-size:13px;">Нажмите на друга в списке слева</p></div></div>`);
  ...
  function renderSubPanel(){
    ...
    document.querySelectorAll(".friend-dots-btn").forEach(btn=>btn.addEventListener("click",e=>{e.stopPropagation();showFriendDotsMenu(btn.dataset.fid,e.clientX,e.clientY)}));
  ...
  function showFriendDotsMenu(fid,x,y){
    const f=S.friendsList.find(fr=>fr.id===fid);if(!f)return;
    const blocked=S.blocked.some(b=>b.id===fid);
    S.ui.showContextMenu([
      {label:`<i class="fa-solid fa-comment"></i> Личные сообщения`,action:()=>openDM(fid)},
      {label:`<i class="fa-solid fa-phone"></i> Позвонить`,action:()=>callUser(f)},
      {label:`<i class="fa-solid fa-copy"></i> Копировать имя`,action:()=>{navigator.clipboard.writeText(f.username||f.display_name);S.ui.toast("Скопировано","info")}},
  ...
  function openDetail(fid){
    ...
    let h=`<div style="display:flex;flex-direction:column;height:100%;">
      ...
      <div class="friend-detail-body">
        ...
        ${blocked?'<div style="text-align:center;color:var(--red);font-weight:600;">🚫 Пользователь заблокирован</div>':`<div style="display:flex;gap:8px;justify-content:center;"><button class="btn btn-primary btn-sm" id="fd-dm-btn"><i class="fa-solid fa-comment"></i> ЛС</button><button class="btn btn-secondary btn-sm" id="fd-call-btn"><i class="fa-solid fa-phone"></i> Звонок</button></div>`}
      </div></div>`;
    ...
    document.getElementById("fd-close")?.addEventListener("click",()=>{showPage()});
    document.getElementById("fd-dm-btn")?.addEventListener("click",()=>openDMView(f));
    document.getElementById("fd-call-btn")?.addEventListener("click",()=>callUser(f))
  }

  function openDMView(friend){
    S.ui.renderChatView({type:"text",receiver_id:friend.id},true,friend.display_name||friend.username);
    S.ui.renderFriendInfo(friend);S.chat.loadDMs(friend.id)
  }
  ...
  async function removeFriend(fid){S.ui.confirm("Удалить друга","Вы уверены?",async()=>{await sb.from("friends").delete().eq("user_id",S.user.id).eq("friend_id",fid);await sb.from("friends").delete().eq("user_id",fid).eq("friend_id",S.user.id);S.ui.toast("Друг удалён","info");await loadAll();showPage()})}
  async function blockUser(uid){S.ui.confirm("Заблокировать","Он не сможет писать и видеть онлайн.",async()=>{await sb.from("blocked_users").insert({blocker_id:S.user.id,blocked_id:uid});await sb.from("friends").delete().eq("user_id",S.user.id).eq("friend_id",uid);await sb.from("friends").delete().eq("user_id",uid).eq("friend_id",S.user.id);S.ui.toast("Заблокирован","info");await loadAll();showPage()})}
  async function unblock(uid){await sb.from("blocked_users").delete().eq("blocker_id",S.user.id).eq("blocked_id",uid);S.ui.toast("Разблокирован","success");await loadAll();renderSubPanel()}
  function callUser(f){const rn=`sentcor_call_${[S.user.id,f.id].sort().join("_")}`;if(S.voice)S.voice.startCall(f,rn)}
  async function acceptReq(rid){const r=S.pending.find(x=>x.id===rid);if(!r)return;await sb.from("friend_requests").update({status:"accepted"}).eq("id",rid);try{await sb.from("friends").insert([{user_id:S.user.id,friend_id:r.sender_id},{user_id:r.sender_id,friend_id:S.user.id}])}catch(e){console.warn("Friend insert:",e.message)}S.ui.toast("Заявка принята!","success");await loadAll();renderSubPanel()}
  ...
  function showAddToServer(fid){if(!S.serversList.length){S.ui.toast("Нет серверов","info");return}let h='<div style="display:flex;flex-direction:column;gap:4px;">';S.serversList.forEach(s=>h+=`<button class="btn btn-ghost" data-sid="${s.id}" style="justify-content:flex-start;">${S.escapeHtml(s.name)}</button>`);h+='</div>';S.ui.showModal("Выберите сервер",h,[{text:"Отмена",cls:"btn-secondary"}]);document.querySelectorAll(".modal .btn-ghost[data-sid]").forEach(btn=>btn.addEventListener("click",async()=>{await sb.from("server_members").insert({server_id:btn.dataset.sid,user_id:fid,role:"member"});S.ui.toast("Приглашён!","success");document.querySelector(".modal-overlay")?.remove()}))}
  function openDM(fid){const f=S.friendsList.find(x=>x.id===fid);if(!f)return;openDetail(fid);setTimeout(()=>document.getElementById("fd-dm-btn")?.click(),150)}

  S.friends={loadAll,showPage,openDetail,openDM,showAddModal,sendReq,blockUser,unblock};
})();