// SENTCOR v6 — UI (diamond logo, fast switch, proper menu)
(function(){
  "use strict";
  ...
  function showHomePage(){
    ...
    let h=`<div class="main-content"><div class="main-header" style="display:flex;align-items:center;gap:8px;padding-bottom:16px;border-bottom:1px solid var(--border);margin-bottom:16px;"><i class="fa-solid fa-house"></i> Главная</div>
      <h2 style="font-size:20px;color:var(--text-bright);">Добро пожаловать, ${esc(p.display_name||p.username||"игрок")}!</h2>
      <p style="color:var(--text-muted);font-size:13px;margin-bottom:24px;">Общайтесь с друзьями в текстовых и голосовых каналах</p>
  ...
  function renderChatView(channel,isDM=false,friendName=""){
    const name=esc(isDM?friendName:channel.name);S.ui.resetCompact();
    const fid=isDM?channel.receiver_id:null;
    const f=isDM?S.friendsList.find(x=>x.id===fid):null;
    const isBlocked=f&&S.blocked.some(b=>b.id===fid);
    let hdr=`<div class="chat-header">
    ...
    // Header buttons
    const callBtn=document.getElementById("chat-call-btn");if(callBtn)callBtn.addEventListener("click",()=>{if(f&&S.voice)S.voice.startCall(f,`sentcor_call_${[S.user.id,f.id].sort().join("_")}`)});
    const menuBtn=document.getElementById("chat-menu-btn");if(menuBtn&&isDM)menuBtn.addEventListener("click",e=>{
      ...
      if(f.muted)items.push({label:'<i class="fa-solid fa-bell"></i> Включить звук',action:()=>{S.friends.toggleMute(fid);S.friends.openChat(fid)}});
      else items.push({label:'<i class="fa-solid fa-bell-slash"></i> Без звука',action:()=>{S.friends.toggleMute(fid);S.friends.openChat(fid)}});
