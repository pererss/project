// SENTCOR v5.2 — UI (Chats tab, fixed nav)
(function(){
  "use strict";
  ...
  async function handleLogin(){$$(".input-error").forEach(e=>e.textContent="");const em=$("#login-email").value.trim(),pw=$("#login-password").value;let v=true;if(!em){$("#login-email-error").textContent="Введите email";v=false}if(!pw){$("#login-password-error").textContent="Введите пароль";v=false}if(!v)return;const btn=$("#login-btn");btn.disabled=true;btn.textContent="Входим...";const{error}=await S.auth.signIn(em,pw);if(error){$("#login-general-error").textContent=getErrorMessage(error);btn.disabled=false;btn.textContent="Войти"}}
  async function handleRegister(){$$(".input-error").forEach(e=>e.textContent="");const un=$("#reg-username").value.trim(),em=$("#reg-email").value.trim(),pw=$("#reg-password").value;let v=true;if(!un||un.length<3){$("#reg-username-error").textContent="Минимум 3 символа";v=false}if(!em){$("#reg-email-error").textContent="Введите email";v=false}if(!pw||pw.length<6){$("#reg-password-error").textContent="Минимум 6 символов";v=false}if(!v)return;const btn=$("#reg-btn");btn.disabled=true;btn.textContent="Создаём...";const{error}=await S.auth.signUp(em,pw,un);if(error){$("#reg-general-error").textContent=getErrorMessage(error);btn.disabled=false;btn.textContent="Создать аккаунт"}else{toast("Аккаунт создан! Проверьте почту.","success",5000);renderLoginForm()}}
  ...
  function bindSidebar(){
    ...
    document.getElementById("nav-chats")?.addEventListener("click",()=>{activateNav("chats");if(S.friends)S.friends.showChatsPage()});
    document.getElementById("nav-friends")?.addEventListener("click",()=>{activateNav("friends");if(S.friends)S.friends.showFriendsPage()});
    document.getElementById("nav-servers")?.addEventListener("click",()=>{activateNav("servers");if(S.servers)S.servers.showPage()});
    document.getElementById("nav-profile")?.addEventListener("click",()=>{activateNav("profile");if(S.profileMod)S.profileMod.showPage()});
    document.getElementById("nav-settings")?.addEventListener("click",()=>{activateNav("settings");if(S.profileMod)S.profileMod.showSettings()});
    document.getElementById("add-server-btn")?.addEventListener("click",()=>{if(S.servers)S.servers.showCreateModal()});
    ...
    document.getElementById("logout-btn")?.addEventListener("click",()=>confirm("Выход","Вы уверены?",async()=>{await S.auth.signOut()}));
  ...
  function showHomePage(){
    ...
    let html=`<div class="main-content"><div class="main-header"><i class="fa-solid fa-house"></i> Главная</div>
      <div style="padding:24px 0 0;"><h2 style="font-size:20px;color:var(--text-bright);margin-bottom:4px;">Добро пожаловать, ${esc(p.display_name||p.username||"игрок")}!</h2>
      <p style="color:var(--text-muted);font-size:13px;margin-bottom:24px;">Общайтесь с друзьями в текстовых и голосовых каналах</p></div>
      <div class="home-section-title">ДРУЗЬЯ В СЕТИ — ${online.length}</div>`;
    if(online.length){online.forEach(f=>{const av=f.avatar_url?`<img src="${f.avatar_url}">`:(f.display_name||f.username||"?").charAt(0).toUpperCase();html+=`<div class="home-friend-card" data-fid="${f.id}" style="cursor:pointer;"><div class="friend-avatar"><div class="avatar">${av}</div><span class="status-dot status-${f.status}"></span></div><div class="friend-info"><div class="friend-name">${esc(f.display_name||f.username)}</div><div class="friend-meta">${f.game_status?esc(f.game_status):"В сети"}</div></div></div>`})}
    else{html+=`<div class="home-empty"><i class="fa-solid fa-users"></i><h3>Никого нет в сети</h3><p>Когда ваши друзья зайдут, они появятся здесь.</p></div>`}
    html+=`</div>`;setMainContent(html);clearMembers();
  ...
  function renderFriendInfo(friend){const sm={online:"🟢 В сети",idle:"🟡 Не активен",dnd:"🔴 Не беспокоить",offline:"⚫ Не в сети"};const av=friend.avatar_url?`<img src="${friend.avatar_url}">`:(friend.display_name||friend.username||"?").charAt(0).toUpperCase();let h=`<div class="sp-section-title">О пользователе</div><div style="padding:12px;text-align:center;"><div class="avatar avatar-lg" style="margin:0 auto;">${av}</div><div style="font-weight:700;font-size:15px;color:var(--text-bright);margin-top:8px;">${esc(friend.display_name||friend.username)}</div><div style="font-size:12px;color:var(--text-muted);">${sm[friend.status]||"⚫ Не в сети"}</div>${friend.game_status?`<div style="font-size:12px;color:var(--green);margin-top:4px;">🎮 ${esc(friend.game_status)}</div>`:""}`;if(friend.custom_status)h+=`<div style="font-size:11px;color:var(--text-secondary);margin-top:4px;">${esc(friend.custom_status)}</div>`;h+=`</div>`;setMembersContent(h)}
