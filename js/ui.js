...
window.S.icons = {
  ...
  kick: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="8.5" cy="7" r="4"/><line x1="18" y1="8" x2="23" y2="13"/><line x1="23" y1="8" x2="18" y2="13"/></svg>'
};
...
window.S.ui.openProfileModal = function(profile) {
  ...
  var html = '<div class="profile-view">' + banner +
    ...
    '<div class="profile-view-meta-item">'+window.S.icons.bell+' Дата регистрации: ' + regDate + '</div>' +
    '</div>' +
  ...
  setTimeout(function(){
    ...
    if(copyBtn) copyBtn.addEventListener('click', function(){
      navigator.clipboard.writeText(profile.id).then(function(){
        ...
        window.S.ui.showToast('Не удалось скопировать','error');
      });
    });
  },0);
};
...
window.S.ui.openProfileEditModal = function(profile, onSave) {
  ...
  var html = '<div class="settings-form">' + avatarHTML +
    ...
    '<div class="settings-group"><label class="settings-label">Тема</label><div class="theme-preview"></div></div>' +
    '</div>';
