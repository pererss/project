// SENTCOR v5.8 — UI (fixed, chat header bar, local messages, stable)
(function(){
  "use strict";
  ...
  function showHomePage(){
    ...
    let h=`<div class="main-content"><div class="main-header"><i class="fa-solid fa-house"></i> Главная</div>
      <div style="padding:24px 0 0;"><h2 style="font-size:20px;color:var(--text-bright);">Добро пожаловать, ${esc(p.display_name||p.username||"игрок")}!</h2>
    ...
    if(online.length){online.forEach(f=>{const av=f.avatar_url?`<img src="${f.avatar_url}">`:(f.display_name||f.username||"?").charAt(0).toUpperCase();h+=`<div class="home-friend-card" data-fid="${f.id}" style="cursor:pointer;"><div class="friend-avatar"><div class="avatar">${av}</div><span class="status-dot status-${f.status}"></span></div><div class="friend-info"><div class="friend-name">${esc(f.display_name||f.username)}</div><div class="friend-meta">${f.game_status?esc(f.game_status):"В сети"}</div></div></div>`})}
    ...
    document.querySelectorAll(".home-friend-card").forEach(c=>c.addEventListener("click",()=>{S.friends.openChat(c.dataset.fid)}))
  }

  // CHAT VIEW with prominent header
  function renderChatView(channel,isDM=false,friendName=""){
    const name=esc(isDM?friendName:channel.name);
    S.ui.resetCompact();
    let hdr=`<div class="chat-header" style="background:var(--bg-primary);border-bottom:2px solid var(--accent);padding:8px 16px;">
      <span style="font-weight:700;font-size:15px;color:var(--text-bright);flex:1;">${isDM?'💬':''} ${name}</span>
      <button class="btn btn-icon btn-ghost btn-sm" id="chat-call-btn" title="Звонок"><i class="fa-solid fa-phone"></i></button>
      <button class="btn btn-icon btn-ghost btn-sm" id="chat-menu-btn" title="Действия"><i class="fa-solid fa-ellipsis-vertical"></i></button>
    </div>`;
    ...
    // Header buttons
    document.getElementById("chat-call-btn")?.addEventListener("click",()=>{if(isDM){const f=S.friendsList.find(x=>x.id===channel.receiver_id);if(f&&S.voice)S.voice.startCall(f,`sentcor_call_${[S.user.id,f.id].sort().join("_")}`)}});
    document.getElementById("chat-menu-btn")?.addEventListener("click",e=>{S.ui.showContextMenu([{label:'<i class="fa-solid fa-trash"></i> Закрыть чат',action:()=>S.ui.showHomePage()}],e.clientX,e.clientY)})
  }
  ...
  // Rename old aliases that servers.js expects
  S.ui={$,$$,toast,showModal,confirm,showContextMenu,closeContextMenu,showAuth,showApp,updateFooter,activateNav,deactivateNav,showHomePage,setSubPanelHeader,renderChatView,appendMessage,renderMembers,renderFriendInfo,clearMembers,isAdmin:false,resetCompact,setMainContent:setMain,setMembersContent:setMembers,setSubPanelContent:setSPContent,getErrorMessage:err=>err?(err.message||String(err)):"Неизвестная ошибка"};
})();