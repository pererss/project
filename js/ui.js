// SENTCOR v12 — Modular UI with Init
(function() {
  "use strict";
  ...
  function init() {
    ...
    // Currently, no specific DOM bindings are needed here as they are
    // triggered by other modules after authentication.
  }
  ...
  function showModal(title, bodyHtml, buttons = []) {
    ...
    if (typeof bodyHtml === "string") bd.innerHTML = bodyHtml;
    else bd.appendChild(bodyHtml);
    m.appendChild(bd);
    if (buttons.length) {
      ...
      buttons.forEach(b => {
        ...
        btn.addEventListener("click", e => {
          if (b.onClick) b.onClick(e, m, o);
          if (b.closeOnClick !== false) o.remove()
        });
        ft.appendChild(btn)
      });
      m.appendChild(ft)
    ...
    const fi = m.querySelector("input");
    if (fi) fi.focus()
  }

  function confirm(title, msg, onConfirm, onCancel) {
    showModal(title, `<p>${msg}</p>`, [{
      text: "Отмена",
      cls: "btn-secondary",
      onClick: () => {
  ...
  function showContextMenu(items, x, y) {
    ...
    m.style.top = Math.min(y, window.innerHeight - (m.scrollHeight || 200) - 10) + "px";
  ...
  function errMsg(err) {
    return err ? (err.message || String(err)) : "Неизвестная ошибка"
  }
  ...
  async function handleLogin() {
    ...
    const em = $("#login-email").value.trim(),
      pw = $("#login-password").value;
    if (!em) {
      $("#login-email-error").textContent = "Введите email";
      return
    }
    if (!pw) {
      $("#login-password-error").textContent = "Введите пароль";
      return
    }
    const btn = $("#login-btn");
    btn.disabled = true;
    ...
    const {
      error
    } = await S.auth.signIn(em, pw);
    if (error) {
      $("#login-general-error").textContent = error.message || "Ошибка";
      btn.disabled = false;
  ...
  async function handleRegister() {
    $$(".input-error").forEach(e => e.textContent = "");
    const un = $("#reg-username").value.trim(),
      em = $("#reg-email").value.trim(),
      pw = $("#reg-password").value;
    if (!un || un.length < 3) {
      $("#reg-username-error").textContent = "Минимум 3 символа";
      return
    }
    if (!em) {
      $("#reg-email-error").textContent = "Введите email";
      return
    }
    if (!pw || pw.length < 6) {
      $("#reg-password-error").textContent = "Минимум 6 символов";
      return
    }
    const btn = $("#reg-btn");
    btn.disabled = true;
    ...
    const {
      error
    } = await S.auth.signUp(em, pw, un);
    if (error) {
      $("#reg-general-error").textContent = error.message || "Ошибка";
      btn.disabled = false;
      btn.textContent = "Создать аккаунт"
    } else {
      safeToast("Аккаунт создан! Проверьте почту.", "success", 5000);
      renderLoginForm()
    }
  }
  ...
  function bindSidebar() {
    ...
    document.getElementById("nav-profile")?.addEventListener("click", () => {
      activateNav("profile");
      if (S.profileMod) S.profileMod.showPage()
    });
    document.getElementById("add-server-btn")?.addEventListener("click", () => {
      S.servers.showCreateModal()
    });
    document.getElementById("sp-toggle")?.addEventListener("click", () => {
      const sp = document.getElementById("sub-panel");
      if (sp) sp.classList.toggle("collapsed")
    });
    document.getElementById("settings-btn")?.addEventListener("click", () => {
      activateNav("");
      if (S.profileMod) S.profileMod.showSettings()
    });
    document.getElementById("footer-avatar")?.addEventListener("click", () => {
      if (S.profileMod) S.profileMod.openMyProfileModal()
    })
  }
  ...
  function updateFooter() {
    ...
    const av = document.getElementById("footer-avatar"),
      un = document.getElementById("footer-username"),
      st = document.getElementById("footer-status"),
      dot = document.getElementById("footer-dot");
  ...
  function setSPHeader(t) {
    const h = document.getElementById("sp-title");
    if (h) h.textContent = t
  }

  function setSPContent(h) {
  ...
  function showHomePage() {
    ...
    if (online.length) {
      online.forEach(f => {
        ...
        h += `<div class="home-friend-card" data-fid="${f.id}"><div class="friend-avatar"><div class="avatar">${av}</div><span class="status-dot status-${f.status}"></span></div><div class="friend-info"><div class="friend-name">${esc(f.display_name||f.username)}</div><div class="friend-meta">${f.custom_status?esc(f.custom_status):(f.status==="online"?"В сети":"Не в сети")}</div></div></div>`
      })
    } else {
      h += `<div class="home-empty"><i class="fa-solid fa-users"></i><h3>Никого нет в сети</h3><p>Когда ваши друзья зайдут, они появятся здесь.</p></div>`
    }
    h += `</div>`;
    setMain(h);
    clearMembers();
    document.querySelectorAll(".home-friend-card").forEach(c => c.addEventListener("click", () => {
      if (S.friends) S.friends.openChat(c.dataset.fid)
    }))
  }

  function renderChatView(channel, isDM = false, friendName = "") {
    const name = esc(isDM ? friendName : channel.name);
    resetCompact();
    const fid = isDM ? (channel.receiver_id === S.user.id ? channel.sender_id : channel.receiver_id) : null;
    const f = isDM ? S.friendsList.find(x => x.id === fid) : null;
    const isBlocked = f && S.blocked && S.blocked.some(b => b.id === fid);
    ...
    else document.getElementById("join-voice-btn")?.addEventListener("click", () => {
      if (S.voice) S.voice.joinChannel(channel)
    });
    document.getElementById("chat-call-btn")?.addEventListener("click", () => {
      if (f && S.voice) S.voice.startCall(f, `sentcor_call_${[S.user.id,f.id].sort().join("_")}`)
    });
    document.getElementById("chat-menu-btn")?.addEventListener("click", e => {
      if (!isDM || !f) return S.ui.showContextMenu([{
        label: '<i class="fa-solid fa-trash"></i> Закрыть',
        action: () => showHomePage()
      }], e.clientX, e.clientY);
      const items = [{
        label: '<i class="fa-solid fa-phone"></i> Позвонить',
        action: () => {
          if (S.voice) S.voice.startCall(f, `sentcor_call_${[S.user.id,f.id].sort().join("_")}`)
        }
      }];
      items.push({
        label: `<i class="fa-solid ${f.muted?'fa-bell':'fa-bell-slash'}"></i> ${f.muted?'Включить звук':'Без звука'}`,
        action: () => {
          S.friends.toggleMute(fid);
          S.friends.openChat(fid)
        }
      });
      items.push({
        label: `<i class="fa-solid ${isBlocked?'fa-lock-open':'fa-ban'}"></i> ${isBlocked?'Разблокировать':'Заблокировать'}`,
        danger: !isBlocked,
        action: () => isBlocked ? S.friends.unblock(fid) : S.friends.blockUser(fid)
      });
      items.push({
        label: '<i class="fa-solid fa-user-minus"></i> Удалить из друзей',
        danger: true,
        action: () => S.friends.removeFriend(fid)
      });
  ...
  function bindChatInput(channel, isDM) {
    const ta = document.getElementById("msg-input"),
      sb = document.getElementById("send-msg-btn");
    if (!ta || !sb) return;
    ...
    newTa.addEventListener("input", () => {
      ...
      newTa.style.height = Math.min(newTa.scrollHeight, 120) + "px"
    });
    ...
    const send = async () => {
      ...
      if (result.error) {
        safeToast("Ошибка отправки: " + result.error.message, "error");
  ...
  function appendMessage(msg, pMap = {}, isDM = false) {
    const ct = document.getElementById("chat-messages");
    if (!ct) return;
    const em = ct.querySelector(".empty-state");
    if (em) em.remove();
    
    const isOwn = msg.sender_id === S.user.id;
    const profile = isOwn ? S.profile : (pMap[msg.sender_id] || {});
    ...
    let statusHTML = msg.edited_at ? `<span class="message-edited" title="${new Date(msg.edited_at).toLocaleString()}">(изм.)</span>` : '';
  ...
  function renderFriendInfo(friend) {
    ...
    const statusText = friend.custom_status || sm[friend.status] || "Не в сети";
    const av = friend.avatar_url ? `<img src="${friend.avatar_url}">` : (friend.display_name || friend.username || "?").charAt(0).toUpperCase();

    const html = `
      <div class="members-header">${esc(friend.display_name || friend.username)}</div>
      <div class="member-card">
        ...
        <div class="friend-info">
          <div class="friend-name">${esc(friend.display_name || friend.username)}</div>
          <div class="friend-meta">${esc(statusText)}</div>
        </div>
      </div>
    `;
    setMembers(html);
