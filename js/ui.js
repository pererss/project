...
(function(){
  ...
  async function handleRegister(){$$(".input-error").forEach(e=>e.textContent="");const un=$("#reg-username").value.trim(),em=$("#reg-email").value.trim(),pw=$("#reg-password").value;if(!un||un.length<3){$("#reg-username-error").textContent="Минимум 3 символа";return}if(!em){$("#reg-email-error").textContent="Введите email";return}if(!pw||pw.length<6){$("#reg-password-error").textContent="Минимум 6 символов";return}const btn=$("#reg-btn");btn.disabled=true;btn.textContent="Создаём...";const{error}=await S.auth.signUp(em,pw,un);if(error){$("#reg-general-error").textContent=error.message||"Ошибка";btn.disabled=false;btn.textContent="Создать аккаунт"}else{if(window.toast&&typeof window.toast.show==="function"){window.toast.show("Аккаунт создан! Проверьте почту.","success",5000)}else{console.warn("Toast not initialized yet")}renderLoginForm()}}
