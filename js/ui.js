...
(function(){
  ...
  function showHomePage(){
    ...
    h+=`</div>`;setMain(h);const sp=S.profile||{};const fr=S.friendsList||[];
    setMembers(`<div class="sp-section-title">Статистика</div><div style="padding:12px;display:flex;flex-direction:column;gap:8px;"><div style="display:flex;justify-content:space-between;"><span style="font-size:12px;">💎 SentCoins</span><span style="font-weight:700;color:var(--yellow);">${(sp.sent_coins||0).toLocaleString()}</span></div><div style="display:flex;justify-content:space-between;"><span style="font-size:12px;">🔥 Стрик</span><span style="font-weight:700;">${sp.streak_days||0} дн.</span></div><div style="display:flex;justify-content:space-between;"><span style="font-size:12px;">👥 Друзей</span><span style="font-weight:700;">${fr.length}</span></div><div style="display:flex;justify-content:space-between;"><span style="font-size:12px;">🖥️ Серверов</span><span style="font-weight:700;">${S.serversList?S.serversList.length:0}</span></div></div>`);
