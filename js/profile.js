// SENTCOR v6 — Profile + Settings (3 tabs, no danger)
(function(){
  ...
  function renderAllInOne(){
    ...
    document.getElementById("avatar-input")?.addEventListener("change",async e=>{const f=e.target.files[0];if(!f||f.size>5*1024*1024){S.ui.toast("Файл >5MB","error");return}const{error}=await S.auth.uploadAvatar(f);if(error)S.ui.toast("Ошибка загрузки","error");else{S.ui.toast("Аватар обновлён!","success");S.ui.updateFooter();renderAllInOne()}});
  ...
  function renderEdit(){
    ...
    S.ui.setMainContent(h);S.ui.clearMembers();
    document.getElementById("save-edit-btn")?.addEventListener("click",async()=>{const dn=document.getElementById("edit-dn").value.trim(),cs=document.getElementById("edit-cs").value.trim();const upd={};if(dn)upd.display_name=dn;upd.custom_status=cs||null;const{error}=await S.auth.updateProfile(upd);if(error)S.ui.toast("Ошибка: "+(error.message||"неизвестная"),"error");else{S.ui.toast("Сохранено!","success");S.ui.updateFooter();renderEdit()}})
  }
  ...
  function renderSettingsTab(tab){
    ...
    }else if(tab==="privacy"){h=`<div class="main-content"><div style="max-width:560px;"><div class="settings-section"><h3>Конфиденциальность</h3><p style="color:var(--text-muted);font-size:12px;">Блокировка — в разделе «Друзья». Заблокированные не видят ваш онлайн и не могут писать.</p></div></div></div>`}
    S.ui.setMainContent(h);S.ui.clearMembers();
    document.querySelectorAll(".theme-option").forEach(el=>el.addEventListener("click",()=>{document.querySelectorAll(".theme-option").forEach(e=>e.classList.remove("active"));el.classList.add("active");const th=el.dataset.th;document.body.classList.remove("theme-oled","theme-midnight");if(th!=="caramel")document.body.classList.add("theme-"+th);S.auth.updateProfile({theme:th})}));
