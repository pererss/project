// SENTCOR v6.1 — Profile (right stats) + Settings (5 themes + custom)
(function(){
  ...
  function renderRightStats(){
    ...
    const h=`<div class="sp-section-title">Обо мне</div><div style="padding:12px;display:flex;flex-direction:column;gap:8px;">
      <div style="display:flex;justify-content:space-between;"><span style="font-size:12px;">💎 SentCoins</span><span style="font-weight:700;color:var(--yellow);">${(p.sent_coins||0).toLocaleString()}</span></div>
      <div style="display:flex;justify-content:space-between;"><span style="font-size:12px;">🔥 Стрик</span><span style="font-weight:700;">${p.streak_days||0} дн.</span></div>
      <div style="display:flex;justify-content:space-between;"><span style="font-size:12px;">💬 Сообщений</span><span style="font-weight:700;">${p.messages_count||0}</span></div>
      <div style="display:flex;justify-content:space-between;"><span style="font-size:12px;">👥 Друзей</span><span style="font-weight:700;">${friends.length}</span></div>
  ...
  function renderEdit(){
    ...
    document.getElementById("save-edit-btn")?.addEventListener("click",async()=>{const dn=document.getElementById("edit-dn").value.trim(),cs=document.getElementById("edit-cs").value.trim();const upd={};if(dn)upd.display_name=dn;upd.custom_status=cs||null;const{error}=await S.auth.updateProfile(upd);if(error)S.ui.toast("Ошибка: "+(error.message||"?"),"error");else{S.ui.toast("Сохранено!","success");S.ui.updateFooter();renderEdit()}})
  }

  // ---- SETTINGS (4 tabs: Внешний вид, Уведомления, Конфиденциальность, Аккаунт) ----
  ...
  function renderSettingsTab(tab){
    ...
    if(tab==="appearance"){
      h=`<div class="main-content"><div style="max-width:560px;"><div class="settings-section"><h3>Тема оформления</h3><div class="theme-selector">
        ...
        <div class="theme-option theme-forest${ct==="forest"?" active":""}" data-th="forest" title="Forest"></div>
        <div class="theme-option theme-rose${ct==="rose"?" active":""}" data-th="rose" title="Rose"></div>
      </div></div>
      <div class="settings-section"><h3>Своя тема</h3><div class="settings-row"><span>Акцент (#HEX)</span><input class="input" id="cust-accent" value="${p.custom_accent||''}" placeholder="#d4853b" style="width:120px;"></div><button class="btn btn-sm btn-primary" id="save-custom-theme">Применить свою тему</button></div></div></div>`
    }else if(tab==="notifications"){
      const soundsOn=p.notify_sounds!==false;
      h=`<div class="main-content"><div style="max-width:560px;"><div class="settings-section"><h3>Уведомления</h3>
        <div class="settings-row"><div><strong>Звук входящих сообщений</strong><div style="font-size:11px;color:var(--text-muted);">Звуковой сигнал при новом ЛС</div></div><label style="display:flex;align-items:center;gap:8px;cursor:pointer;"><input type="checkbox" id="notify-sounds" ${soundsOn?"checked":""}> Вкл</label></div>
        <button class="btn btn-sm btn-primary" id="save-notify-btn"><i class="fa-solid fa-floppy-disk"></i> Сохранить</button>
      </div></div></div>`
    }else if(tab==="privacy"){
      h=`<div class="main-content"><div style="max-width:560px;"><div class="settings-section"><h3>Конфиденциальность</h3>
        <p style="font-size:12px;color:var(--text-muted);margin-bottom:12px;">Кто может отправлять вам личные сообщения</p>
        <div class="settings-row"><select class="input" id="priv-dm"><option value="friends" ${p.dm_privacy==="friends"?"selected":""}>Только друзья</option><option value="all" ${p.dm_privacy!=="friends"?"selected":""}>Все</option></select></div>
        <button class="btn btn-sm btn-primary" id="save-privacy-btn"><i class="fa-solid fa-floppy-disk"></i> Сохранить</button>
      </div></div></div>`
    }else if(tab==="account"){
      ...
      h=`<div class="main-content"><div style="max-width:560px;"><div class="settings-section"><h3>Аккаунт</h3>
        ...
        <div style="display:flex;gap:8px;margin-top:12px;"><button class="btn btn-primary btn-sm" id="save-acc-btn"><i class="fa-solid fa-floppy-disk"></i> Сохранить</button><button class="btn btn-danger btn-sm" id="logout-settings-btn"><i class="fa-solid fa-right-from-bracket"></i> Выйти</button></div>
      </div></div></div>`
    }
    ...
    // Custom theme save
    document.getElementById("save-custom-theme")?.addEventListener("click",()=>{const accent=document.getElementById("cust-accent")?.value||"#d4853b";const ct=JSON.stringify({accent});document.body.style.setProperty("--accent",accent);document.body.classList.add("theme-custom");S.auth.updateProfile({theme:"custom",custom_accent:accent,custom_theme:ct})});
    // Notifications save
    document.getElementById("save-notify-btn")?.addEventListener("click",()=>{const sounds=document.getElementById("notify-sounds")?.checked;S.auth.updateProfile({notify_sounds:sounds});S.ui.toast("Сохранено!","success")});
    // Privacy save
    document.getElementById("save-privacy-btn")?.addEventListener("click",()=>{const dm=document.getElementById("priv-dm")?.value;S.auth.updateProfile({dm_privacy:dm});S.ui.toast("Сохранено!","success")});
    // Account save
    document.getElementById("save-acc-btn")?.addEventListener("click",async()=>{const game=document.getElementById("st-game").value,cs=document.getElementById("st-cstatus").value.trim();const upd={};upd.game_status=game?`Играет в ${game}`:null;upd.custom_status=cs||null;const{error}=await S.auth.updateProfile(upd);if(error)S.ui.toast("Ошибка","error");else{S.ui.toast("Сохранено!","success");S.ui.updateFooter()}});
