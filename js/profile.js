// SENTCOR v9 — Profile (Bio + Thought Bubble + Modal)
(function(){
  ...
  function renderAllInOne(){const p=S.profile;renderMainProfile();S.ui.clearMembers()}
  function renderEdit(){const p=S.profile;let h=`<div class="main-content"><div style="max-width:560px;"><div class="input-group"><label class="input-label">Отображаемое имя</label><input class="input" id="edit-dn" value="${S.escapeHtml(p.display_name||"")}" placeholder="${S.escapeHtml(p.username||"Имя")}" maxlength="50"></div><div class="input-group"><label class="input-label">О себе</label><textarea class="input" id="edit-bio" rows="3" maxlength="200">${S.escapeHtml(p.bio||"")}</textarea></div><div class="input-group"><label class="input-label">Пользовательский статус</label><input class="input" id="edit-cs" value="${S.escapeHtml(p.custom_status||"")}" placeholder="Ваш статус..." maxlength="100"></div><button class="btn btn-primary btn-sm" id="save-edit-btn"><i class="fa-solid fa-floppy-disk"></i> Сохранить</button></div></div>`;S.ui.setMainContent(h);S.ui.clearMembers();document.getElementById("save-edit-btn")?.addEventListener("click",async()=>{const dn=document.getElementById("edit-dn").value.trim(),bio=document.getElementById("edit-bio").value.trim(),cs=document.getElementById("edit-cs").value.trim();const upd={};if(dn)upd.display_name=dn;upd.bio=bio||null;upd.custom_status=cs||null;const{error}=await S.auth.updateProfile(upd);if(error)S.ui.toast("Ошибка","error");else{S.ui.toast("Сохранено!","success");S.ui.updateFooter();renderEdit()}})}

  function renderMainProfile(){
    const p=S.profile;const av=p.avatar_url?`<img src="${p.avatar_url}">`:(p.display_name||p.username||"?").charAt(0).toUpperCase();const since=new Date(p.created_at).toLocaleDateString("ru-RU",{year:"numeric",month:"long",day:"numeric"});
    let h=`<div class="main-content"><div class="profile-card"><div class="profile-banner"></div><div class="profile-avatar-area"><div class="avatar avatar-xl">${av}</div><div><div class="profile-name">${S.escapeHtml(p.display_name||p.username)}</div><div class="profile-game">${S.escapeHtml(p.bio||"")}</div></div></div>
      ...
      <div style="margin-top:16px;"><button class="btn btn-sm btn-primary" id="avatar-upload-btn"><i class="fa-solid fa-camera"></i> Аватар</button><input type="file" id="avatar-input" accept="image/*" hidden><button class="btn btn-sm btn-secondary" id="goto-edit-btn"><i class="fa-solid fa-pen"></i> Редактировать</button></div></div></div></div>`;
  ...
  // ---- OPEN PROFILE MODAL (click footer avatar) ----
  async function openMyProfileModal(){
    await S.auth.fetchProfile();const p=S.profile;const isMe=true;renderProfileModal(p,isMe)
  }

  async function openUserProfileModal(userId){
    const{data:pr}=await sb.from("profiles").select("*").eq("id",userId).maybeSingle();
    if(!pr){S.ui.toast("Пользователь не найден","error");return}
    const isMe=pr.id===S.user.id;renderProfileModal(pr,isMe)
  }
  ...
  function renderProfileModal(p,isMe){
    ...
    let html=`<div style="max-width:520px;padding:8px 0;">
      <div style="display:flex;align-items:flex-start;gap:16px;margin-bottom:20px;">
        <div class="avatar avatar-xl" style="flex-shrink:0;">${av}</div>
        <div style="flex:1;min-width:0;padding-top:4px;">
          <div style="font-weight:800;font-size:20px;color:var(--text-bright);">${S.escapeHtml(p.display_name||p.username)}</div>
          <div style="font-size:13px;color:var(--text-muted);margin-top:2px;">@${S.escapeHtml(p.username)}</div>
          <div style="font-size:12px;color:var(--text-muted);margin-top:2px;">С нами с ${since}</div>
        </div>
      </div>
      <!-- Thought Bubble -->
      <div style="position:relative;background:var(--bg-tertiary);border:1px solid var(--border);border-radius:16px;padding:14px 16px;margin-bottom:16px;min-height:44px;">
        <div style="position:absolute;top:-12px;left:24px;width:0;height:0;border-left:10px solid transparent;border-right:10px solid transparent;border-bottom:12px solid var(--bg-tertiary);"></div>`;
    if(isMe&&hasThought){
      html+=`<div style="display:flex;align-items:flex-start;justify-content:space-between;"><span style="font-size:13px;color:var(--text-primary);flex:1;">${thoughtText}</span><button class="btn btn-icon btn-ghost btn-sm" id="delete-thought-btn" title="Удалить"><i class="fa-solid fa-trash"></i></button></div>`
    }else if(isMe){
      html+=`<span id="thought-text" style="font-size:13px;color:var(--text-muted);cursor:pointer;">${thoughtText}</span>`
    }else{
      html+=`<span style="font-size:13px;color:${hasThought?'var(--text-primary)':'var(--text-muted)'};">${thoughtText}</span>`
    }
    html+=`</div>`;
    // Bio
    html+=`<div class="input-group" style="margin-bottom:16px;"><label class="input-label">О себе</label>`;
    if(isMe){html+=`<textarea class="input" id="modal-bio" rows="2" maxlength="200">${S.escapeHtml(p.bio||"")}</textarea><button class="btn btn-sm btn-primary" id="save-bio-btn" style="margin-top:8px;"><i class="fa-solid fa-floppy-disk"></i> Сохранить</button>`}
    else{html+=`<p style="font-size:13px;color:var(--text-primary);">${S.escapeHtml(p.bio||"Не указано")}</p>`}
    html+=`</div>`;
    // Avatar upload (only me)
    ...
    const{overlay}=S.ui.showModal("Профиль",html,[{text:"Закрыть",cls:"btn-secondary"}]);

    // Thought click → edit
    if(isMe){
      const thoughtEl=document.getElementById("thought-text");if(thoughtEl)thoughtEl.addEventListener("click",()=>{
        const existing=document.querySelector("#thought-edit");if(existing)return;
        const inp=document.createElement("input");inp.className="input";inp.id="thought-edit";inp.value=p.thought||"";inp.placeholder="Ваша мысль на 24ч...";inp.maxLength=200;
    ...
    // Save bio
    const saveBioBtn=document.getElementById("save-bio-btn");if(saveBioBtn)saveBioBtn.addEventListener("click",async()=>{const bio=document.getElementById("modal-bio").value.trim();await S.auth.updateProfile({bio:bio||null});S.ui.toast("Сохранено!","success");overlay.remove();openMyProfileModal()});
  ...
  function renderSettingsTab(tab){const p=S.profile;const ct=p.theme||"caramel";let h="";
    ...
    else if(tab==="account"){const can=S.auth.canChangeUsername(),days=S.auth.daysUntilUsernameChange();h=`<div class="main-content"><div style="max-width:560px;"><div class="settings-section"><h3>Аккаунт</h3><div class="settings-row"><div><strong>Имя: ${S.escapeHtml(p.display_name||p.username)}</strong></div></div><div class="settings-row"><div><strong>Сменить имя</strong><div style="font-size:11px;color:var(--text-muted);">${can?"Доступно":`Недоступно (${days} дн.)`}</div></div>${can?`<button class="btn btn-sm btn-secondary" id="ch-username-btn"><i class="fa-solid fa-pen"></i> Сменить</button>`:""}</div><div class="settings-row"><div><strong>Смена пароля</strong></div><button class="btn btn-sm btn-secondary" id="reset-pw-btn"><i class="fa-solid fa-key"></i> Сменить</button></div><div style="display:flex;gap:8px;margin-top:12px;"><button class="btn btn-primary btn-sm" id="save-acc-btn"><i class="fa-solid fa-floppy-disk"></i> Сохранить</button><button class="btn btn-danger btn-sm" id="logout-settings-btn"><i class="fa-solid fa-right-from-bracket"></i> Выйти</button></div></div></div></div>`}
    S.ui.setMainContent(h);S.ui.clearMembers();
  ...
  function showStatusSelector(){return} // Disabled
